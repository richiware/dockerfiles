...
COPY st-stlink-server-2.1.1-1-linux-amd64.deb /tmp/st-stlink-server-2.1.1-1-linux-amd64.deb
...
RUN sudo -E LICENSE_ALREADY_ACCEPTED=1 dpkg -i /tmp/st-stlink-server-2.1.1-1-linux-amd64.deb && sudo rm /tmp/st-stlink-server-2.1.1-1-linux-amd64.deb
